...
// todo: replace with String and Number
const columnTypeToJsType = {
...
const processFields = fields => {
	...
	if ( newFields ) newFields = newFields.map( field => {
		...
		const columnType = columnTypeToJsType[ COLUMN_TYPES[ field.columnType ] ] ||
			throwF( `Missing type (${field.columnType}, ${COLUMN_TYPES[ field.columnType ]})` );
...
const appendResults = ( results, rows, fields ) => {
	...
	if ( Array.isArray( rows ) )
		return results.push( { rows, fields: processFields( fields ) } );
